...
CREATE TABLE ##Pointers (Phase BIGINT, Pointer INT)
...
INSERT ##Pointers (Phase, Pointer) VALUES (5,0), (6,0), (7,0), (8,0), (9,0)
...
BEGIN
    ...
    WHILE @Output <> -99
    BEGIN
        EXEC @Output = IntCodeComp @Output, @AmpA
        --PRINT @Output
        EXEC @Output = IntCodeComp @Output, @AmpB
        --PRINT @Output
        EXEC @Output = IntCodeComp @Output, @AmpC
        --PRINT @Output
        EXEC @Output = IntCodeComp @Output, @AmpD
        --PRINT @Output
        EXEC @Output = IntCodeComp @Output, @AmpE
        --PRINT @Output
        INSERT ##Results (ThrusterID, Result) SELECT @ID, @Output

    END

    UPDATE ##Thrusters
    SET OutputValue = (SELECT MAX(Result) FROM ##Results WHERE ThrusterID = @ID)
    WHERE ID = @ID
